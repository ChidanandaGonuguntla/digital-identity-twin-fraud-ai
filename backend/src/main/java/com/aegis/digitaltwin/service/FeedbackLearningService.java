...
public class FeedbackLearningService {
    ...
    public CustomerTwin applyFeedback(FeedbackRequest request) {
        CustomerEvent event = eventRepository.findByEventId(request.eventId())
        ...
        feedbackRepository.save(FraudFeedback.builder()
        ...
        if (request.outcome() == FeedbackOutcome.LEGITIMATE || request.outcome() == FeedbackOutcome.FALSE_POSITIVE) {
            if (event.getDeviceId() != null && !twin.getKnownDevices().contains(event.getDeviceId())) twin.getKnownDevices().add(event.getDeviceId());
            if (event.getLocation() != null && !twin.getKnownLocations().contains(event.getLocation())) twin.getKnownLocations().add(event.getLocation());
            if (event.getMerchant() != null && !twin.getTrustedMerchants().contains(event.getMerchant())) twin.getTrustedMerchants().add(event.getMerchant());
            if (event.getPayee() != null && !twin.getTrustedPayees().contains(event.getPayee())) twin.getTrustedPayees().add(event.getPayee());

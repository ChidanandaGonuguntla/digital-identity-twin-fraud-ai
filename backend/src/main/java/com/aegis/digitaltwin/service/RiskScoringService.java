...
public class RiskScoringService {
    private static final Set<String> HIGH_RISK_CATEGORIES = Set.of("CRYPTO", "GAMBLING", "WIRE", "CASH_OUT");
    private static final Set<String> HIGH_RISK_IP_PREFIXES = Set.of("102.", "185.", "45.");

    public FraudDecisionResponse evaluate(String eventId, CustomerTwin twin, ActivityEventRequest request) {
        int score = 0;
        ...
        if (request.merchant() != null && !containsIgnoreCase(twin.getTrustedMerchants(), request.merchant())) {
        ...
        if (request.payee() != null && !request.payee().isBlank() && !containsIgnoreCase(twin.getTrustedPayees(), request.payee())) {
        ...
        if (request.merchantCategory() != null && HIGH_RISK_CATEGORIES.contains(request.merchantCategory().toUpperCase())) {
        ...
        if (request.ipAddress() != null && HIGH_RISK_IP_PREFIXES.stream().anyMatch(request.ipAddress()::startsWith)) {
        ...
        return new FraudDecisionResponse(eventId, request.customerId(), score, decision, reasons, signalScores, Instant.now());
    }

...
COMMENT
ON TABLE  customer_twin            IS 'Behavioral digital twin: per-customer running baseline used for deviation scoring.';
COMMENT
ON COLUMN customer_twin.baseline   IS 'JSONB: {hourHistogram[24], categoryCounts{}, lastLat, lastLon, lastTs}.';
...
SELECT customer_id,
       ...
       CASE
           WHEN transaction_count > 0
               THEN ROUND(amount_sum / transaction_count, 2)
           ELSE 0 END AS mean_amount,
       CASE
           WHEN transaction_count > 1
               THEN ROUND(SQRT(GREATEST(amount_sum_sq / transaction_count
                                            - POWER(amount_sum / transaction_count, 2), 0))::numeric, 2)
           ELSE 0 END AS std_dev_amount,
       last_updated
FROM customer_twin;
...
COMMENT
ON TABLE  ml_fraud_score                      IS 'Per-transaction fraud decision: deterministic rule score + ML probability fused into a final decision.';
COMMENT
ON COLUMN ml_fraud_score.ml_fraud_probability IS 'Model output is a probability only; it never decides on its own.';
COMMENT
ON COLUMN ml_fraud_score.final_decision       IS 'APPROVE / REVIEW / DECLINE, from the deterministic Risk Fusion Engine.';
